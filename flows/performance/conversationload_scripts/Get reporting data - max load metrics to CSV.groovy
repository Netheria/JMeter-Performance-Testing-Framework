...
public class GlobalVariableManager {
    ...
    */
    ...
    public static void initialize(String cataloguePath,
        ...
        scriptName = "Get reporting data - max load metrics to CSV";
...
if (args.size() < EXPERCTED_ARGS_LEN) {
    ...
    Logger.logWriter("Error", "Skipping Max Load metrics gathering: " +
    ...
    // MAIN FLOW START
Logger.logWriter("Info", "Started Max Load metrics gathering.");
...
if (props.get("Test Max load") != null) {
    ...
    Integer time_maxload_s = (Integer) ((props.get("START.MS") as long) / 1000L);
    dateTimeMaxload = Instant.ofEpochSecond(time_maxload_s).toString();
}

int time_end_s = props.get("Test End");
String dateTimeEnd = Instant.ofEpochSecond(time_end_s).toString();
...
String dfilePath = GlobalVariableManager.getCataloguePath() +
                   ...
                   GlobalVariableManager.getBuildTag() + "/MaxLoad/";
File dfile = null;
    ...
    type.graph.each { graph ->
        ...
        graph.group.each { group ->
            ...
            String query = group.query.text().replaceAll('[\\t\\n\\r]', '')
                .replaceAll("dateTimeStart", dateTimeMaxload)
                .replaceAll("dateTimeEnd", dateTimeEnd)
...
Logger.logWriter("Info", "Finished Max Load metrics gathering.");
